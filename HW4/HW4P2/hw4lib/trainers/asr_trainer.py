...
class ASRTrainer(BaseTrainer):
    ...
    def __init__(self, model, tokenizer, config, run_name, config_file, device=None):
        ...
        # Use blank_id for CTC blank token (not pad_id!)
        self.ctc_weight = self.config['loss'].get('ctc_weight', 0.0)
        if self.ctc_weight > 0:
            self.ctc_criterion = nn.CTCLoss(
                blank=self.tokenizer.blank_id,  # Fixed: use blank_id, not pad_id
                zero_infinity=True
            )
        else:
    ...
    def _validate_epoch(self, dataloader):
        ...
        if recognition_config is None:
            # Create a default config that processes all batches
            recognition_config = {
                'num_batches': None,  # Process all batches during validation
                'beam_width': 1,
                'temperature': 1.0,
                'repeat_penalty': 1.0,
                'lm_weight': 0.0,
            ...
            # Ensure we process all batches during validation
            recognition_config = recognition_config.copy()
            recognition_config['num_batches'] = None
            # Handle YAML None values (YAML may load None as string "None" or "null")
            if recognition_config.get('lm_model') in [None, "None", "null", ""]:
                recognition_config['lm_model'] = None
            # Generate config_name based on beam_width
            beam_width = recognition_config.get('beam_width', 1)
            if beam_width == 1:
                config_name = 'greedy'
            else:
                config_name = f'beam_{beam_width}'
        
        results = self.recognize(dataloader, recognition_config, config_name=config_name)
    ...
    def train(self, train_dataloader, val_dataloader, epochs: int):
        ...
        for epoch in range(self.current_epoch, self.current_epoch + epochs):
            ...
            # Save attention plots (guard when train_attn is None)
            if isinstance(train_attn, dict) and len(train_attn):
    ...
    def recognize(self, dataloader, recognition_config: Optional[Dict[str, Any]] = None, config_name: Optional[str] = None, max_length: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        # Convert num_batches from string to int if needed (YAML may load as string)
        if isinstance(recognition_config.get('num_batches'), str):
            if recognition_config['num_batches'].lower() in ['none', 'null', '']:
                recognition_config['num_batches'] = None
            else:
                recognition_config['num_batches'] = int(recognition_config['num_batches'])
        ...
        with torch.inference_mode():
            for i, batch in enumerate(dataloader):
                ...
                if targets_golden is not None:
                    # Trim targets to actual lengths and remove EOS before decoding
                    targets_list = []
                    for j in range(targets_golden.size(0)):
                        seq = targets_golden[j][:transcript_lengths[j].item()]  # Remove padding
                    ...
                    # Decode per-sample targets (not the padded batch)
                    for j, (pred, target) in enumerate(zip(post_processed_preds, targets_list)):
                        results.append({
