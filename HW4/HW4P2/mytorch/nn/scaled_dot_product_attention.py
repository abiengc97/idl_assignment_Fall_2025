...
class ScaledDotProductAttention:
    ...
    def forward(self, Q, K, V, mask=None):
        ...
        scaled_dot_product = self.Q @ self.K.swapaxes(-2, -1)
        ...
        self.attention_scores = self.softmax.forward(scaled_dot_product/np.sqrt(self.K.shape[-1]))

        # Calculate output: (N, ..., H, L, Ev)
        # (N, ..., H, L, S) @ (N, ..., H, S, Ev) -> (N, ..., H, L, Ev) 
        output = self.attention_scores @ self.V
