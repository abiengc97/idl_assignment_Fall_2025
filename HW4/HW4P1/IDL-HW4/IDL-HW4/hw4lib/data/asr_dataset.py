...
class ASRDataset(Dataset):
    def __init__(
        ...
        self.fbank_dir   = os.path.join(config['root'], 'fbank', partition)
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        batch_feats  = [torch.FloatTensor(feat) for feat in batch]
        ...
        if self.partition != "test-clean":
            ...
            batch_shifted      = [torch.tensor(transcript) for transcript in batch_shifted] # B x T
            batch_golden       = [torch.tensor(transcript) for transcript in batch_golden] # B x T
        ...
        return padded_feats, padded_shifted, padded_golden, torch.tensor(feat_lengths), torch.tensor(transcript_lengths)
