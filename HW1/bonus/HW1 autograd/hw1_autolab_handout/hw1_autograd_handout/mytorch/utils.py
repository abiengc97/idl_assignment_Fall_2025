...
As discussed in the writeup (Section: 3.2.3 GradientBuffer Class), we track two types of gradients in different ways.
...
class GradientBuffer:
    ...
    def add_spot(self, np_array):
        if not self.is_in_memory(np_array):
            ...
            assert (self.memory[self.get_memory_loc(np_array)].shape == np_array.shape), "You cannot add the same array with different views to the buffer."
    ...
    def update_param(self, np_array, gradient):
        ...
        if type(gradient).__name__ == "NoneType":
            ...
            raise Exception("Attempted to add gradient for a variable not in gradient buffer.")

    def get_param(self, np_array):
        if self.is_in_memory(np_array):
            return self.memory[self.get_memory_loc(np_array)]
        else:
            print(f"Not found array: {np_array}")
            raise Exception("Attempted to get gradient for a variable not in gradient buffer.")

    def set_param(self, np_array, gradient):
        if self.is_in_memory(np_array):
            self.memory[self.get_memory_loc(np_array)] = gradient
        else:
            raise Exception("Attempted to set gradient for a variable not in gradient buffer.")
