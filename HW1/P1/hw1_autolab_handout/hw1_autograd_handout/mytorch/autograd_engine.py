...
class Autograd:
    ...
    def add_operation(self,
        ...
        Args:
            ...
            - backward_operation: backward function for nn/functional.py.
                        When passing a function you don't need inputs or parentheses.
        Additional Tips:
            - The number of gradients_to_update must equal the number of inputs.
            - Inputs should be in the same state as during the operation. Additionally, 
                        The input's gradient should be in the same state as well
                        ex: If you are multiplying matrices and one of them is transposed, you should pass the transposed input instead of the original one.
                            Additionally, the gradients of the input should be transposed as well, unless it is a None.
            - In case a gradient does not need to be updated, you can enter it as None.
            - Inputs, outputs and gradients_to_update should all be numpy arrays. Integer values passed as inputs, should be converted to numpy arrays before being passed into the function.
        Returns:
            No return required
        """
    ...
    def backward(self, divergence):
        ...
        the gradients_to_update. Read the writeup for further explanation
        Args:
        ...
        # NOTE: Make sure you iterate in the correct direction. How are gradients propagated?
            ...
            # For the remaining iterations the gradient to be propagated can be retrieved from the self.gradient_buffer.get_param.
            ...
            # Check with the Operation's gradients_to_update if you need to directly update a gradient, and do the following accordingly:
            #   1) Inputs with internally tracked gradients: update the gradient stored in self.gradient_buffer
            #   2) Inputs with externally tracked gradients: update gradients_to_update.
            #   Update them by incrementing the proper gradients_to_update by grad.
