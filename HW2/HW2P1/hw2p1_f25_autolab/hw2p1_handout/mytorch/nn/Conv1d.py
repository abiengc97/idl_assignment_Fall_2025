...
class Conv1d_stride1():
    ...
    def forward(self, A):
        ...
        """
        #use tensordot to compute the convolution with kernel size 
    ...
    def backward(self, dLdZ):
        ...
        """
        #matrix convolution of A and dLdZ
        ...
        padded_dL_dZ = np.pad(dLdZ, ((0, 0), (0, 0), (self.kernel_size-1, self.kernel_size-1)), 'constant')
        flipped_W = np.flip(self.W, axis=2)
        for i in range(self.A.shape[2]):
            dLdA[:,:,i]=np.tensordot(padded_dL_dZ[:, :, i:i+self.kernel_size], flipped_W, axes=([1, 2], [0, 2]))
...
class Conv1d():
    ...
    def forward(self, A):
        ...
        A = np.pad(A, ((0, 0), (0, 0), (self.pad, self.pad)), 'constant')
