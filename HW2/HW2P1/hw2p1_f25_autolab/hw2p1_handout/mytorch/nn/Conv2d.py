    ...
    def backward(self, dLdZ):
        ...
        for i in range(self.kernel_size):
            for j in range(self.kernel_size):
                self.dLdW[:, :, i, j] = np.tensordot(dLdZ, self.A[:, :, i:i+output_height, j:j+output_width], axes=([0, 2, 3], [0, 2, 3]))
        ...
        for i in range(self.A.shape[2]):
            for j in range(self.A.shape[3]):
                dLdA[:, :, i, j] = np.tensordot(padded_dLdZ[:, :, i:i+self.kernel_size, j:j+self.kernel_size], flipped_W, axes=([1, 2, 3], [0, 2, 3]))
...
class Conv2d():
    ...
    def forward(self, A):
        ...
        A = np.pad(A, ((0, 0), (0, 0), (self.pad, self.pad), (self.pad, self.pad)), 'constant')
